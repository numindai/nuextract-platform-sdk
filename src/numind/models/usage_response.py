...
class UsageResponse(BaseModel):
    ...
    period: DateRange
    template_usage: UsageItemResponse = Field(
        description="Template extraction usage", alias="templateUsage"
    )
    structured_usage: UsageItemResponse = Field(
        description="Structured data extraction usage", alias="structuredUsage"
    )
    content_usage: UsageItemResponse = Field(
        description="Content extraction usage", alias="contentUsage"
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of template_usage
        if self.template_usage:
            _dict["templateUsage"] = self.template_usage.to_dict()
        # override the default output from pydantic by calling `to_dict()` of structured_usage
        if self.structured_usage:
            _dict["structuredUsage"] = self.structured_usage.to_dict()
        # override the default output from pydantic by calling `to_dict()` of content_usage
        if self.content_usage:
            _dict["contentUsage"] = self.content_usage.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "period": DateRange.from_dict(obj["period"])
                if obj.get("period") is not None
                else None,
                "templateUsage": UsageItemResponse.from_dict(obj["templateUsage"])
                if obj.get("templateUsage") is not None
                else None,
                "structuredUsage": UsageItemResponse.from_dict(obj["structuredUsage"])
                if obj.get("structuredUsage") is not None
                else None,
                "contentUsage": UsageItemResponse.from_dict(obj["contentUsage"])
                if obj.get("contentUsage") is not None
                else None,
            }
        )
        return _obj
