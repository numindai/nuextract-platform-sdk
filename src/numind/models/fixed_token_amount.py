...
class FixedTokenAmount(BaseModel):
    """
    FixedTokenAmount
    """
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["fixed-amount"]):
            raise ValueError("must be one of enum values ('fixed-amount')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {"amount": obj.get("amount"), "type": obj.get("type")}
        )
        return _obj
