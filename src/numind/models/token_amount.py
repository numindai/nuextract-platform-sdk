...
TOKENAMOUNT_ONE_OF_SCHEMAS = ["FixedTokenAmount", "UnlimitedTokenAmount"]
...
class TokenAmount(BaseModel):
    ...
    # data type: FixedTokenAmount
    oneof_schema_1_validator: Optional[FixedTokenAmount] = None
    # data type: UnlimitedTokenAmount
    oneof_schema_2_validator: Optional[UnlimitedTokenAmount] = None
    actual_instance: Optional[Union[FixedTokenAmount, UnlimitedTokenAmount]] = None
    one_of_schemas: Set[str] = {"FixedTokenAmount", "UnlimitedTokenAmount"}
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = TokenAmount.model_construct()
        error_messages = []
        match = 0
        # validate data type: FixedTokenAmount
        if not isinstance(v, FixedTokenAmount):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `FixedTokenAmount`"
            )
        else:
            match += 1
        # validate data type: UnlimitedTokenAmount
        if not isinstance(v, UnlimitedTokenAmount):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `UnlimitedTokenAmount`"
            )
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in TokenAmount with oneOf schemas: FixedTokenAmount, UnlimitedTokenAmount. Details: "
                + ", ".join(error_messages)
            )
        if match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in TokenAmount with oneOf schemas: FixedTokenAmount, UnlimitedTokenAmount. Details: "
                + ", ".join(error_messages)
            )
        return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into FixedTokenAmount
        try:
            instance.actual_instance = FixedTokenAmount.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into UnlimitedTokenAmount
        try:
            instance.actual_instance = UnlimitedTokenAmount.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into TokenAmount with oneOf schemas: FixedTokenAmount, UnlimitedTokenAmount. Details: "
                + ", ".join(error_messages)
            )
        if match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into TokenAmount with oneOf schemas: FixedTokenAmount, UnlimitedTokenAmount. Details: "
                + ", ".join(error_messages)
            )
        return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], FixedTokenAmount, UnlimitedTokenAmount]]:
