...
class UsageItemResponse(BaseModel):
    ...
    api_usage: UsageMetricsResponse = Field(
        description="API usage (billable)", alias="apiUsage"
    )
    interface_usage: UsageMetricsResponse = Field(
        description="Interface usage (non-billable)", alias="interfaceUsage"
    )
    __properties: ClassVar[List[str]] = ["apiUsage", "interfaceUsage"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.api_usage:
            _dict["apiUsage"] = self.api_usage.to_dict()
        # override the default output from pydantic by calling `to_dict()` of interface_usage
        if self.interface_usage:
            _dict["interfaceUsage"] = self.interface_usage.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "apiUsage": UsageMetricsResponse.from_dict(obj["apiUsage"])
                if obj.get("apiUsage") is not None
                else None,
                "interfaceUsage": UsageMetricsResponse.from_dict(obj["interfaceUsage"])
                if obj.get("interfaceUsage") is not None
                else None,
            }
        )
        return _obj
