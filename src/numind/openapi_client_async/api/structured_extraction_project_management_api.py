...
class StructuredExtractionProjectManagementApi:
    ...
    async def post_api_structured_extraction_structuredprojectid_reset_settings(
        ...
        post_api_structured_extraction_structuredprojectid_reset_settings
         ...
         Reset the settings of an existing **Structured Extraction Project** to their default values.  Default values are:   Setting | Default | -----------|---------|  `temperature` | 0.6 |  `rasterizationDPI` | 170|  `maxOutputTokens` | 0 (no limit) |  `degradedMode`  (deprecated) | Reject|  `maxExampleTokenNumber` (former `maxTokensSmartExample`) | 90000|  #### Error Responses: `404 Not Found` - If a **Project** with the specified `projectId` does not exist.  `403 Forbidden` - If the user does not have permission to update this **Project**.  `403 Locked` - If the **Project** is locked.
