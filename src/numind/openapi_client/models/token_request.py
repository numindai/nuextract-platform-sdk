...
class TokenRequest(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, TokenCodeRequest):
            error_messages.append(f"Error! Input type `{type(v)}` is not `TokenCodeRequest`")
        else:
            match += 1
        # validate data type: TokenRefreshRequest
        if not isinstance(v, TokenRefreshRequest):
            error_messages.append(f"Error! Input type `{type(v)}` is not `TokenRefreshRequest`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in TokenRequest with oneOf schemas: TokenCodeRequest, TokenRefreshRequest. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in TokenRequest with oneOf schemas: TokenCodeRequest, TokenRefreshRequest. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into TokenRequest with oneOf schemas: TokenCodeRequest, TokenRefreshRequest. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into TokenRequest with oneOf schemas: TokenCodeRequest, TokenRefreshRequest. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], TokenCodeRequest, TokenRefreshRequest]]:
