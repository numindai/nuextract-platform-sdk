...
[**get_api_documents_documentid_parts_partindex_image**](DocumentsApi.md#get_api_documents_documentid_parts_partindex_image) | **GET** /api/documents/{documentId}/parts/{partIndex}/image | 
...
# **get_api_documents_documentid_parts_partindex_image**
> bytes get_api_documents_documentid_parts_partindex_image(document_id, part_index, x_organization_id=x_organization_id)

Returns the image content of a specific part, by its logical index among image-capable parts.
...
------------- | ------------- | ------------- | -------------
 **document_id** | **str**| Unique document identifier. | 
 **part_index** | **int**| Zero-based logical index among image-capable parts of the document. | 
 **x_organization_id** | **str**| Optional organization to use for this request.   No header means that the user personal account will be used.   This token is *only* used by the _frontend_ application and *will be ignored if used with the API*. When using the api, the organization used will be the one of the api key. | [optional] 
