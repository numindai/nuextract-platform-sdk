"""
RFC 3987 verification method.

This code is based on the rfc3987 package: https://codeberg.org/atufi/rfc3987
And has been made stricter to handle more specific edge cases.
"""
...
_uri_rules = (
    ...
    # reg_name is a dot-separated sequence of labels. Prevents '..' and leading/trailing
    # dots.
    ("reg_name", r"{reg_label}(?:\.{reg_label})*\.?"),
    # A label is a sequence of non-dot characters.
    ("reg_label", r"(?:[a-zA-Z0-9_~-]|{pct_encoded}|{sub_delims})+"),
    ########   PATH   ########
    (
        "path",
        (
            r"(?:{path_abempty}|{path_absolute}|{path_noscheme}"
...
_iri_rules = (
    ...
    # ireg_name is a dot-separated sequence of labels. Prevents '..' and
    # leading/trailing dots.
    ("ireg_name", r"{ireg_label}(?:\.{ireg_label})*\.?"),
    # A label is a sequence of non-dot characters.
    ("ireg_label", r"(?:[a-zA-Z0-9_~-]|{ucschar}|{pct_encoded}|{sub_delims})+"),
    ########   PATH   ########
    (
        "ipath",
        (
...
def format_patterns(**names) -> dict:
    r"""
    Return a dict of patterns (regular expressions).

    The regexes are keyed by `rule names for URIs`_ and `rule names for IRIs`_.
    ...
    for name, pat in _common_rules[::-1] + _uri_rules[::-1] + _iri_rules[::-1]:
        if name in names:
            n = names[name]
            pat = n(pat) if callable(n) else f"(?P<{n}>{pat})"
...
DEFAULT_GROUP_NAMES = dict(
    zip(_GROUP_NAMES_BASE, _GROUP_NAMES_BASE),
    path_abempty="path",
    path_absolute="path",
    path_noscheme="path",
    path_rootless="path",
    path_empty="path",
    ipath_abempty="ipath",
    ipath_absolute="ipath",
    ipath_noscheme="ipath",
    ipath_rootless="ipath",
    ipath_empty="ipath",
)
...
def _interpret_unicode_escapes(string: str) -> str:
...
upatterns_no_names = {
    k: _interpret_unicode_escapes(v) for k, v in format_patterns().items()
}


def _bmp(s: str) -> str:
    return _re.sub(r"\\U[0-9A-F]{8}-\\U[0-9A-F]{8}", "", s)


#: patterns restricted to the basic multilingual plane
#: compilable on narrow build
bmp_patterns = {k: _bmp(v) for k, v in patterns.items()}
#: compilable on narrow build with re
bmp_upatterns_no_names = {
    k: _interpret_unicode_escapes(_bmp(v)) for k, v in patterns_no_names.items()
}


def get_compiled_pattern(rule: str, flags: int = 0) -> _re.Pattern:
    """
    Return a compiled pattern object for a rule name or template string.
...
def match(string: str, rule: str = "IRI_reference") -> dict | None:
    """
    Check if `string` matches a specific rule.
    ...
    """
    return get_compiled_pattern(f"^%({rule})s$").match(string)
...
def parse(string: str, rule: str = "IRI_reference") -> dict:
    """
    Parse `string` according to `rule` into a dict of subcomponents.
...
def compose(
    ...
    iauthority: str | None = None,
    ipath: str | None = None,
    iquery: str | None = None,
    ifragment: str | None = None,
    **kwargs,
) -> str:
    """
    Return an URI composed_ from named parts.
